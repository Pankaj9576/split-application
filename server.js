...
app.get('/api/proxy', async (req, res) => {
  ...
  try {
    ...
    if (contentType.includes('text/html') && targetUrl.includes('patents.google.com/patent')) {
      ...
      // Updated Application Events Logic
      const applicationEvents = [];

      // 1. Try timeline events
      ...
      // 2. Map legalEvents and metadata if timeline empty
      if (applicationEvents.length === 0) {
        // Mapping legalEvents codes to applicationEvents titles
        const legalEventMap = {
        ...
        if (publicationDate) {
          ...
          // Check for prior publication (e.g., US20120091551A1)
